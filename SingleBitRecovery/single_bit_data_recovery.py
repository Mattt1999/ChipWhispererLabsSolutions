import numpy as np
import random # to generate random input data
...
#performs the encryption using the key secret_key
def aes_secret(inputdata):
    secret_key = 0xEF
    return aes_internal(secret_key, inputdata)

#return the value of a certain bit from the input:
def get_bit(data, bit):
    return ((data & (0x01<<bit))>>bit)

#function to count how many elements are the same between two lists (of the same size)
def num_same(a, b):
...
# function aes_leakage is just get_bit applied to aes_internal
def aes_leakage_guess(keyguess, inputdata, bit):
    return get_bit(aes_internal(keyguess, inputdata), bit)
    
#generate the input data - 1000 random bytes - so values between 0 and 255
input_data = [random.randint(0, 255) for _ in range(0, 1000)]

#get the lowest - most rightmost - byte from the input_data fed through the aes_secret function
leaked_data = [(aes_secret(a) & 0x01) for a in input_data]

# final loop, where we don't know exactly which bit is the leakage - so check all of them
for bit_guess in range(0, 8):
    ...
    for guess in range(0, 256):
        ...
        hypothetical_leakage = [aes_leakage_guess(guess, a, bit_guess) for a in input_data]
